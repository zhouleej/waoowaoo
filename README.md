...
灵感视频沿用同一创作入口：能力目录声明支持纯文字生成的 Veo / 移动云 Seedance 模型可以不上传主图，其他模型仍需上传。历史支持复用素材和参数、失败重试、取消、删除及分页。生成结果会探测实际尺寸和时长；不支持的视频文件会明确失败。无图协议已通过隔离测试，实际账号能否调用取决于服务商权限。

本次新增镜头独立模型字段，已有 MySQL 部署升级前运行 `npx tsx --env-file=.env scripts/migrations/add-panel-video-model.ts`，再运行 `npx prisma generate` 并重启网页与 Worker。此迁移只新增可空字段，可重复执行，不删除旧数据。新部署可按既有流程初始化数据库。
