...
本次新增镜头独立模型字段，已有 MySQL 部署升级前运行 `npx prisma migrate deploy --schema prisma/schema.prisma`，再运行 `npx prisma generate` 并重启网页与 Worker。此迁移只新增可空字段，可重复执行，不删除旧数据。新部署可按既有流程初始化数据库。
